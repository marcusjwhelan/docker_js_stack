...
resource "google_service_account" "terraform1" {
  project      = var.project
  account_id   = "${var.cluster_name}-terraform-admin-sa"
  display_name = "Terraform1 service account"
}
resource "google_service_account_key" "terraform1_admin_key" {
  service_account_id = google_service_account.terraform1.name
}
# add iam policy bindings to terraform service account in gcp
resource "google_project_iam_member" "terraform-source_admin" {
  project = var.project
  role    = "roles/source.admin"
  member  = "serviceAccount:${google_service_account.terraform1.email}"
}
